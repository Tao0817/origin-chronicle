...
- 発見メモUI改善
- 中世・近世データ拡充（+68件）
- データ総件数：388件（正本: src/data/events.json）
...
npm start
```
### 2026-05-28 追記（Codex確認）

- `src/data/events.json` の実データ件数は **388件**。これが現在の正しい件数。
- 以前の記録「370件」は古い。370件からさらに **18件追加済み**。
- 最新コミット確認: `3312d4a feat: Wikidata・WHE照合による年号修正4件＋新規追加18件`
- 正本データは `src/data/events.json`。件数確認時は作業メモではなく、このJSONを直接読むこと。
- 2026-05-28時点の年代分布: 古代78件 / 中世63件 / 近世53件 / 近現代194件 / 合計388件。
